...
const API_Gateway_Sli_Slo = ({slidata}) => {
    ...
    useEffect(()=>{
        function extract() {
            ...
            for (let i = 0; i < slidata.length; i++) {
              ...
              if (slidata[i].facet[0].includes("4XX AND 5XX Errors")) {
                const newfacet = slidata[i].facet[0].split(' ').slice(0, -1).join(" ")
                // slidata[i].facet[0] = newfacet
                const newObj = { status: "4XX AND 5XX Errors",newfacet, ...slidata[i] }
                newArr.push(newObj)
              }
...
export default API_Gateway_Sli_Slo